...
const EventRegistration = ({ activities }) => {
  ...
  const totalRegister = activities.registeredUsers.length-1;
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const userDataArray = await Promise.all(activities.registeredUsers.map((userID) => fetchUserData(userID)));
